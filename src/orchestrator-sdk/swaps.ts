...
export default async function main({
  ...
  const result = (await fetch("https://li.quest/v1/quote", {
    method: "POST",
    body: JSON.stringify({
      fromChain: targetChain.id,
      toChain: targetChain.id,
      fromToken: getTokenAddress("USDC", targetChain.id),
      toToken: getTokenAddress("WETH", targetChain.id),
      fromAmount: 2n,
      fromAddress: targetSafeAccount.address,
      allowExchanges: "lifidexaggregator,sushiswap",
    }),
    headers: {
    ...
    targetExecutions: [
      {
        ...
        data: encodeFunctionData({
          ...
          args: [
            result.data.estiamte.approvalAddress,
            BigInt(result.data.action.fromAmount),
          ],
        }),
      },
      {
        to: result.data.transactionRequest.to,
        value: BigInt(result.data.transactionRequest.value),
